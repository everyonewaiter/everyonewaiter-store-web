...
function MainMenuPage() {
  ...
  const handleOpenCategoryModal = () => {
    overlay.open((overlayProps) => <CategoryModal {...overlayProps} categories={data ?? []} />);
  };
  ...
  if (!isLoading && !data?.length) return <CategoryEmptyState />;
  ...
  return (
    <div className="hide-scrollbar w-full overflow-x-hidden px-5 py-4.5 md:px-0">
      ...
      <div className="flex w-full shrink-0 flex-col justify-between gap-5 py-5 md:pt-6 md:pb-2 lg:flex-row lg:items-center">
        {isLoading ? (
          <Skeleton className="h-10 rounded-xl md:h-9 lg:h-10 lg:p-1" />
        ) : (
            ...
            <Button
              ...
              responsiveButtons={{
                ...
                md: {
                  buttonSize: "custom",
                  className:
                    "rounded-xl shrink-0 size-9 bg-gray-700 items-center justify-center p-0",
                },
                sm: {
                  buttonSize: "custom",
                  className:
          ...
          <>
            {isChangedToMenuOrder ? (
              <div className="flex items-center justify-end gap-2">
                <div className="text-primary hidden h-9 items-center justify-center rounded-lg bg-[#F220200A] px-4 text-xs font-normal md:flex">
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-4 py-4 md:grid-cols-4 md:gap-x-3.5 lg:gap-x-6.5 lg:gap-y-10">
          {new Array(6)
            ...
            .map((index) => (
              <Skeleton
                key={index}
                className="relative aspect-152/210 cursor-pointer overflow-hidden rounded-xl md:aspect-159/220 lg:aspect-329/440 lg:rounded-3xl"
              />
            ))}
        </div>
      ) : (
        <DragList
          ...
          prepend={
            !isChangedToMenuOrder && !isLoading ? (
              <button
                type="button"
