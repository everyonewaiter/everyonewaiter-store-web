...
export default function Loading() {
  const isLottieSupported = globalThis?.window !== undefined;

  useEffect(() => {
    if (globalThis.window === undefined) return;

    const originalOverflow = globalThis.window.document.body.style.overflow;
    globalThis.window.document.body.style.overflow = "hidden";

    return () => {
      globalThis.window.document.body.style.overflow = originalOverflow;
    };
  }, []);
